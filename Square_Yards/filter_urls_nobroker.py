...
#     """
#     Filters URLs based on keywords:
#     - If only one keyword -> return all URLs containing it.
#     - If multiple keywords -> only keep URLs that contain *all* keywords.
#     """
...
#         word = keyword_words[0].lower()
#         return [u for u in urls if word in u]

#     # Case 2: Multiple keywords -> all must match
#     keyword_words = [w.lower() for w in keyword_words]
#     return [u for u in urls if all(w in u for w in keyword_words)]
...
BASE_URL = "./"
city_name = "bangalore"
input_file = f"{BASE_URL}{city_name}/query_{city_name}_nb.json"
input_file = f"{BASE_URL}{city_name}/sources/nobroker.ndjson"
...
cm.save_json(f"./{city_name}/unique_urls_sqy_nobroker.json", unique_urls)
