...
city_name = "bangalore"
input_file = f"./{city_name}/builders_{city_name}.ndjson"
current_url = f"https://www.squareyards.com/projects-by-brigade-in-bangalore"
output_file = f"./{city_name}/projects_{city_name}_sy.ndjson"
progress_file = f"./{city_name}/progress_{city_name}_sy.json"
...
def save_progress(progress_page, current_url, page):
    """Update and save last scraped page number"""
    progress_page[current_url] = page
...
def main():
    ...
    last_done_page = progress_page.get(current_url, 0)
    ...
    try:
        ...
        print(f"Error on page {last_success+1}: {e}")
    finally:
        # save only once (on exit or completion)
        save_progress(progress_page, current_url, last_success)
        print(f"Progress saved → Last completed page: {last_success}")
