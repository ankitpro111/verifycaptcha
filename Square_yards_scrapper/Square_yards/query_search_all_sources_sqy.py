...
def google_search(query: str, num_results: int = 30):
    ...
    steps = [1, 11, 21]  # always 3 calls for 30 results
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(steps), desc=f"Fetching '{query}'"):
...
input_file = f"projects_{city_name}_sy.ndjson"
output_file = f"query_{city_name}_all_sources_sqy.ndjson"
...
try:
    for idx, row in enumerate(not_fetched_record[:total_rows], start=1):
        ...
        query_searches.add(f'site:housing.com {query_text}') #"-/in/buy/projects/page -/buy/"
        query_searches.add(f'site:magicbricks.com {query_text}')
        query_searches.add(f'site:99acres.com {query_text}') #""
        query_searches.add(f'site:nobroker.in {query_text}') #""
